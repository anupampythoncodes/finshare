...
export default function Login() {
  ...
  const Login = async (e) => {
    ...
    try {
      ...
      const token = response.data.token; // ⬅️ Make sure backend sends this
      if (!token) {
        throw new Error("No token received from server");
      }

      localStorage.setItem("authToken", token); // ⬅️ Save token
      console.log("Logged in successfully");
      navigate("/account"); // ⬅️ Go to a protected page
    } catch (err) {
